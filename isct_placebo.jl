...
# ╔═╡ 7e58f894-c6a3-4cdb-9c5e-f3618bae68c3
using DifferentialEquations, Random, DataFrames, Distributions

# ╔═╡ 91191512-69de-11f1-8b1c-6b9378852b36
md"""

We have earlier shown that if we are able to identify the clearance rate of a drug (parameter k, and specify an initial concentration on the basis of dose), and a possible action, then we can show that with a one sample pre-post intervention, what can we expect. This time, we will use the same logic but we will show what happens when we add a placebo effect.

"""

# ╔═╡ a04b63d3-e331-4056-b9cf-797b5585f9d9
md"""
First we load the packages

"""

# ╔═╡ 7ebb1dab-10fe-488b-956d-b8b9fadfc6d7
Random.seed!(123)

# ╔═╡ 196f783e-ee33-48d2-a3af-4905529d79b9
md"""

Define an ordinary differential equation where we show how concentration of the drug will drop. This is a presumed or hypothesised model of how the drug concentration drops. In real life you will need to study the pharmacokinetic models to define the model. We have deliberately kept it simple enough for our purpose here. 
"""

# ╔═╡ 20f2d219-c1da-4c50-9696-83c52b38f2aa
function blood_pressure_ode(u, p, t)
    k, Emax, EC50, BP_baseline = p
    return -k * u  # Returns a scalar derivative
end

# ╔═╡ 3d7a6fdb-faa1-4b07-949a-3c5bfb5a8963
md"""

Then we have defined how blood pressure will change as we apply the drug. We have hypothesised that the drug will have a maximum effect per concentration, and it will also have an effect at 50% concentration in terms of how much the blood pressure will change. So, when the drug will act, we know that the blood pressure will change. 

"""

# ╔═╡ 65668639-8d50-46c4-b331-0f307fc406d1
...
# ╔═╡ c9be058d-dd19-4e91-bd92-fc0572927c8f
begin
	dist_BP_baseline = Normal(140, 10)
dist_k           = LogNormal(log(0.15), 0.2)
dist_Emax        = Uniform(25, 45)
dist_EC50        = Uniform(5, 15)
end

# ╔═╡ c7138b28-b4eb-419e-9d7e-10370678c550
num_patients = 500 # number of patients in each arm is set at 500

# ╔═╡ 2f7608fe-4f04-4d3b-b034-888ad751bd8e
# create an empty array of patients
virtual_cohort = []

# ╔═╡ 613cee02-d579-4a04-9f67-f498cfa5d1c3
# now populate that array with data
...
# ╔═╡ 2650e59e-f2ac-43f9-97b2-c5a183090210
# define the trial protocol, we will measure for 24 hours 
tspan = (0.0, 24.0)

# ╔═╡ 8efc41aa-5b76-4a7d-9fe3-f3027e591165
#define the trial protocol, we will start with 50 mg/L of the drug
u0 = 50.0

# ╔═╡ d23ddc58-38d3-495c-ab6f-fba965d3fe26
# Now create an empty results data frame
results = DataFrame(
    Patient_ID = Int[], 
    Arm = String[], 
    Baseline_BP = Float64[], 
...
# ╔═╡ 20ec0ab3-995a-4c14-8d98-b4927435e80d
md"""

Note that in that data frame, we have patient ID, which ARM the patient belongs to, a baseline BP that is randomly assigned to something like 140 mmHg as mean and some defined standard deviation, a final BP that will be determined based on the drug action, and another variable that tells us how much is the BP reduction or effect size. 

"""

# ╔═╡ 0a3e78f9-a955-496e-9d73-5334b5e36491
# now run the algorithms for drug and placebo together
for i in 1:num_patients
    p_drug = virtual_cohort[i]
    bp_baseline = p_drug[4]
    
    # --- DRUG ARM ---
    prob_drug = ODEProblem(blood_pressure_ode, u0, tspan, p_drug)
    sol_drug = solve(prob_drug, Tsit5(), saveat=24.0)
    C_final_drug = sol_drug.u[end]
    bp_final_drug = calculate_bp(C_final_drug, p_drug)
    
    push!(results, (i, "Drug", bp_baseline, bp_final_drug, bp_baseline - bp_final_drug))
    
    # --- PLACEBO ARM ---
    # Create identical patient parameters but force Emax (index 2) to 0.0
    p_placebo = [p_drug[1], 0.0, p_drug[3], p_drug[4]]
    
    prob_placebo = ODEProblem(blood_pressure_ode, u0, tspan, p_placebo)
    sol_placebo = solve(prob_placebo, Tsit5(), saveat=24.0)
    C_final_placebo = sol_placebo.u[end]
    bp_final_placebo = calculate_bp(C_final_placebo, p_placebo)
    
    push!(results, (i, "Placebo", bp_baseline, bp_final_placebo, bp_baseline - bp_final_placebo))
end

# ╔═╡ 8fdd12c2-8e27-4462-aaf1-117c1cb67a9e
# if you now take a look at first five and last five rows of results
first(results, 5)

# ╔═╡ 250ece3c-0ec6-4d2f-b81d-97a7676ab3e6
md"""
You can see that placebo has not done any effect at all, hence no drop. There is a variability ino how much drop we have experienced in the drug arm. We certainly can change the placebo parameters and set lower values based on what we know, so instead of 0, we can have some values as happen in traditional treatment which are more commonly used. 
"""

# ╔═╡ a3968aee-200e-448d-ab5b-e75eb05d402f
# filter the rows for drug and placebo arms
begin
	drug_arm = filter(row -> row.Arm == "Drug", results)
placebo_arm = filter(row -> row.Arm == "Placebo", results)
end

# ╔═╡ 798d362a-0e14-4820-99e8-44c63108f4d9
# Now check the drop of bp in each arm
begin
...
println("-"^50)
println("COMPARATIVE TRIAL RESULTS SUMMARY:")
println("-"^50)
println("Drug Arm Drop:    ", round(mean_drug_drop, digits=2), " ± ", round(std_drug_drop, digits=2), " mmHg")
println("Placebo Arm Drop: ", round(mean_placebo_drop, digits=2), " ± ", round(std_placebo_drop, digits=2), " mmHg")
println("Net Drug Effect:  ", round(mean_drug_drop - mean_placebo_drop, digits=2), " mmHg")
println("-"^50)
end

# ╔═╡ 7d3016f1-8683-4dc9-aad9-d1c0be818631
md"""

OK, so this was a more or less contrived example, where we have set the effect size of placebo to be 0. In real life it is less likely to be so. We can certainly assign other values to the placebo arm and test the efficacy of the treatment arm. In the next tutorial, we will look into how we can add small effects to the "placebo" or control arm to study the differences
