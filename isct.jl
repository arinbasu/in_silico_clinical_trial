...
# ╔═╡ eea3c240-dd3b-481b-80d0-80072e0426ea
using DifferentialEquations, Random, DataFrames, Distributions

# ╔═╡ b1941ea6-69d1-11f1-89b1-71e357ec50ac
md"""
## In silico clinical trials

In silico clinical trials are computer simulations of clinical trials where an evidence based analyst can test the safety and efficacy of a pharmaceutical agent. These clinical trials are not done on real humans but on simulated virtual patients. Here we are going to simulate such clinical trials

"""

# ╔═╡ 48732b5e-447c-41da-85ad-659244f1ab37
# Make it reproducible
Random.seed!(123)

# ╔═╡ 006dd7d6-c526-4d89-9022-1cbac1c5066d
md"""
We are setting up an ordinary differential equation where we state that the pharmaceutical agent has a clearance dC/dt = -C * k where k is the parameter that will reduce the concentration of the drug over time. 

"""

# ╔═╡ 8840c9b4-b007-4423-b133-1548f93047ff
function blood_pressure_ode(u, p, t)
    # C = u[1]  # Current drug concentration
    k, Emax, EC50, BP_baseline = p
    
    # PK: Drug clearance over time
    return -k * u
end

# ╔═╡ d00411a2-1c71-47b8-8510-c0e415c4904e
md"""

Then we write another equation where we will say how the blood pressure will be affected by the drug as the C takes effect. That C will have a maximum effect Emax and it will have an effect at a point where 50% of the people with the disease will experience benefit EC50, so at any given point in time, once the drug is administered, the blood pressure will be lower than some baseline by Emax * C (that is maximum effect / concentration times the concentration divided by EC50 plus the concentration)

"""

# ╔═╡ 17a0aaf7-d5f3-4e00-af7d-33ce5169d8af
...
# ╔═╡ 174d7473-b576-4064-81d6-61c3067af70a
md"""

Now let's create some population distributions. These will normally be derived from the literature.

"""

# ╔═╡ 2dd4c1e7-9af8-4b13-90f0-c4d77e783469
dist_BP_baseline = Normal(140, 10) #blood pressure of the affected population

# ╔═╡ 3fd7193e-8d7e-402d-858a-c823fa824053
dist_k           = LogNormal(log(0.15), 0.2) # the clearance of the drug

# ╔═╡ 13c26270-1c56-4d97-8a25-5a36741d377e
dist_Emax        = Uniform(25, 45) # that the drug can reduce the blood pressure between 25 and 45 mmHg

# ╔═╡ 8b917a8a-00cb-4d43-acaa-560a29709271
dist_EC50        = Uniform(5, 15)    # Concentration for half-max effect (mg/L)

# ╔═╡ b182b3aa-8c34-4d7a-85b2-2123cb97cdcf
md"""

So we have proposed that our drug can reduce the blood pressure anywhere between 25 and 45 mmHg in our previous experiments, we also state that the concentration of the drug when it works at half the efficacy is between 5 and 15 mg/L, and we have stated that our drug will be cleared from the body at the rate of 0.15 mg/L in a log Normal distribution, and we assume that for our affected population, the average blood pressure will be about 140 mmHg, with a standard deviation of about 10 mmHg (normally these will be based on studies and laboratory data)

Now let's say we create a virtual population of about 1000 people with these parameters on whom we will run the clinical trial. 

"""

# ╔═╡ 7a7c4458-a378-46a3-ad82-e2cd6952fffc
num_patients = 1_000

# ╔═╡ 7442030d-5d03-4390-99f5-bdb8aecf5a6e
# create a virtual cohort
virtual_cohort = []

# ╔═╡ c3665691-b8eb-452f-b278-827cdf4eb9e6
md"""

We are assembling a virtual cohort of 1000 patients who have randomly distributed parameters that we have mentioned above.

"""

# ╔═╡ 2ef4be1f-6856-48ed-a174-80c8b300d16a
for i in 1:num_patients
    patient_params = [
        rand(dist_k),
        rand(dist_Emax),
        rand(dist_EC50),
        rand(dist_BP_baseline)
    ]
    push!(virtual_cohort, patient_params)
end

# ╔═╡ a75e38b0-6adb-4e0b-9eca-773f3ea3082f
md"""
Now that we have 1000 patients, we will track each patient for a period of 24 hours. We will measure the blood pressure of that patient each hour, and we will administer an initial oral dose of 50 mg/L and then follow through as the drug diminishes in concentration, what happens to the blood pressure. 

"""

# ╔═╡ 627077f0-102a-402c-98b0-5071ffe594ea
# first the time span
tspan = (0.0, 24.0)

# ╔═╡ 88df86d0-5c3b-4bc9-aa2b-6cbf7348a538
u0 = 50.0 # initial concentration 50 mg/L

# ╔═╡ f089e031-32e1-44d7-9719-1aa8d1fc071f
#  create an empty results data set
results = DataFrame(
    Patient_ID = Int[], 
    Baseline_BP = Float64[], 
    Final_BP = Float64[], 
...
# ╔═╡ c03396be-bd5a-4e8a-9b80-444b5646d5e0
# Now have 1000 (or number of patients) baseline blood pressure
for i in 1:num_patients
    p = virtual_cohort[i]
    bp_baseline = p[4]
end

# ╔═╡ d76ff231-58b6-4f4f-bc28-3e4560d90154
p = virtual_cohort[1]

# ╔═╡ 1fea77bd-6d17-4ac3-aa65-812629717268
# set up the ode
begin
	prob = ODEProblem(blood_pressure_ode, u0, tspan, p)
    sol = solve(prob, Tsit5(), saveat=24.0)
end

# ╔═╡ 32577252-e968-4f5f-a307-3c4e4fcf9473
C_final = sol.u[end]

# ╔═╡ 64f42e6c-668d-48a5-8400-392546764b6c
# the final bp
bp_final = calculate_bp(C_final, p)

# ╔═╡ a5a91554-d55c-4a10-88b3-9154f177820b
for i in 1:num_patients
    p = virtual_cohort[i]
    bp_baseline = p[4]
    
    # Set up and solve the ODE for the individual patient
    prob = ODEProblem(blood_pressure_ode, u0, tspan, p)
    sol = solve(prob, Tsit5(), saveat=24.0) # Save data at the final 24-hour mark
    
    # Extract final concentration and calculate final blood pressure
    C_final = sol.u[end][1]
    bp_final = calculate_bp(C_final, p)
    
    push!(results, (i, bp_baseline, bp_final, bp_baseline - bp_final))
end

# ╔═╡ 639926a9-dfa3-4669-86a1-41d07bf1ba5d
begin
	mean_baseline = mean(results.Baseline_BP)
mean_final    = mean(results.Final_BP)
mean_drop     = mean(results.BP_Reduction)
std_drop      = std(results.BP_Reduction)
	[mean_baseline, mean_final, mean_drop, std_drop]
end

# ╔═╡ 1c9d0e18-13f0-44a3-88b9-3f336f384c0e
# Let's find the non-responders (let's say who had less than 5 mmHg reduction)
non_responders = count(x -> x < 5.0, results.BP_Reduction)

# ╔═╡ deb5e68e-6be0-4631-b3f5-ff57e18e863d
begin
	println("-"^45)
println("TRIAL RESULTS SUMMARY:")
println("-"^45)
println("Average Baseline BP:  ", round(mean_baseline, digits=2), " mmHg")
println("Average Final BP:     ", round(mean_final, digits=2), " mmHg")
println("Average BP Reduction: ", round(mean_drop, digits=2), " ± ", round(std_drop, digits=2), " mmHg")
println("Non-responder Count:  ", non_responders, " out of ", num_patients)
println("-"^45)
end

# ╔═╡ 9cef846d-b414-4a64-a41a-0d563e10bb3a
md"""

So, the above is an in-silico trial where we have taken a known drug with a known clearance value from laboratory data and we have applied the drug to a known cohort of 1000 people with defined blood pressure level, and we have simulated the effect of the drug as to what would happen in 24 hours time if that drug with the presumed effect of anywhere between 25 and 45 mmHg blood pressure reduction. In the next example we will show what happens when we add a placebo arm for the similar situation. This example can be extended to any situation where a firm can conduct a small study on the parameters of drug clearance for example (k value as shown here), and a possible reduction in the blood pressure on a set of volunteers, and this can then be simulated to 1000s of patients with similar profile to get an idea as to what to expect in terms of reduction in blood pressure. In our next example, we will extend this example to comparative studies of a drug and a placebo. 
...
manifest_format = "2.0"
project_hash = "738a572795f46febc3f26430f629dc117bf6e413"
...
git-tree-sha1 = "8361c03a1088cee83d7efd9dcf76dd74da59a9d9"
...
git-tree-sha1 = "bedf94fdda31530604410a1702219545b0044955"
