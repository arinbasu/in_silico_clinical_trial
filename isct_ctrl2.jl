...
# ╔═╡ e0b92991-bca9-4227-a678-46acee19dae0
using Distributions, Random, DifferentialEquations, DataFrames

# ╔═╡ efca9982-6a67-11f1-9560-476fecf2b0f8
md"""

Earlier we have seen that in silico controlled trials can be conducted with one sample pre-post studies and where we had placebo. 

"""

# ╔═╡ 529ace4f-cffa-445a-b30c-129d72daa003
# Generate random see
Random.seed!(123)

# ╔═╡ 464762c8-0d0e-49c0-8e76-ad4695a2065c
function blood_pressure_ode(u,p,t)
	k,Emax,EC50,BP_baseline = p
	return -k * u
end

# ╔═╡ cd632b4e-1f35-44d6-a521-7dd9ece9fdef
...
# ╔═╡ 6178250d-44ea-448f-ab30-185c28db928a
# Generate virtual population parameters
begin
...
# ╔═╡ c3b12769-9260-4caa-ba76-00bab76d851b
# Drug versus Control
begin
...
# ╔═╡ 85b26da1-d4bb-4ae0-a637-da68ac61f20c
# Create virtual cohorts
begin
n_patients = 500
virtual_cohort = []
end

# ╔═╡ d567c041-4dc2-4b39-a1bc-c167b8224aea
...
# ╔═╡ b2ccea66-1f6c-49d7-b9b2-73fe07c2a5a4
# Now simulate
for i in 1:n_patients
...
# ╔═╡ 0a7aa059-dc71-43a6-905b-ba8932c9e35b
# Separate drug and placebo arms
begin
	drug_arm = filter(row -> row.Arm == "Drug", results)
    placebo_arm = filter(row -> row.Arm == "Placebo", results)
end

# ╔═╡ c8f59e6f-bd1f-4313-9e22-dbfe472f45e5
# Generate the report
begin
...
println("-"^50)
println("COMPARATIVE TRIAL RESULTS (ACTIVE PLACEBO):")
println("-"^50)
...
# ╔═╡ 14066b3a-1d21-4d3d-bcf9-ac4e0eba3f9d
md"""

So, in the above, we set the Emax for the placebo or the control to lower than that of the intervention and we found that no surprise that the effect of the drug was stronger than that of the control arm.
So far, we have seen what happens with our in silico modelling when we have a single sample on which we use the known parameters of drug clearance, Emax - the maxium effect size of drop in blood pressure, EC50 or the reduction in blood pressure was similar or same for both active and controls and on this basis, we measured the drop on blood pressure from baseline (which was similar across both groups at 140 mmHg average and about 10 mmHg standard deviation) and then we measured the difference between two groups.
While these are somewhat artificial examples, we still need to understand that in real life, we do not always report difference in means. Rather, we report relative risk, where we state the relative likelihood of reaching the desired end points for the two groups (active drug versus placebo). This effect measure is referred to as relative risk. In our next example, we will work out the relative risk values. 
